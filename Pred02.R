...
df_pred1 = aggregate(data.frame(df_sales4$item_cnt_month), by=data.frame(df_sales4$shop_id, df_sales4$item_id), sum, na.rm=TRUE)
colnames(df_pred1) = c("shop_id", "item_id", "item_cnt_month")
df_pred1 = merge(df_pred1, df_items[2:3], by= "item_id") # merge variable item_category

# sum by shop and item_category
df_pred1c = aggregate(data.frame(df_pred1$item_cnt_month), by=data.frame(df_pred1$shop_id, df_pred1$item_category_id), sum, na.rm=TRUE)
...
df_pred3$item_cnt_month = round(df_pred3$item_cnt_month/34, digits=1)
...
############# Submit ###################
subfile = paste(paste(Pathcsv, "Test3_SumMonthsDiv34", sep=""), ".csv", sep="")
write.csv(df_pred3[4:5], row.names=FALSE, file=subfile)
